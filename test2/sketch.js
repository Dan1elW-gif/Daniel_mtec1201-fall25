...
function preload() {
  ...
  Idrive = createVideo (['video/Idrive.mp4']);
  Idrive.hide()
...
function playGame() {
  ...
  // 3. RYAN CAR AI 
  let totalFlee = createVector(0, 0);
  let dToCop = dist(ryanPos.x, ryanPos.y, copPos.x, copPos.y);
  let dToMax = dist(ryanPos.x, ryanPos.y, maxPos.x, maxPos.y);

  // RYAN FLEE
  if (dToCop < 150) {
    let fCop = p5.Vector.sub(ryanPos, copPos);
    fCop.normalize().mult(1000 / (dToCop + 1));
    totalFlee.add(fCop);
  }

  // FLEE (MAX)
  if (s > 17 && dToMax < 150) {
    let fMax = p5.Vector.sub(ryanPos, maxPos);
    fMax.normalize().mult(1000 / (dToMax + 1));
    totalFlee.add(fMax);
  }
  ...
  // MARGIN TO THE WALL
  let margin = 60;
  let wallPush = createVector(0, 0);
  if (ryanPos.x < margin) wallPush.x = 1;
  if (ryanPos.x > width - margin) wallPush.x = -1;
  if (ryanPos.y < margin) wallPush.y = 1;
  if (ryanPos.y > height - margin) wallPush.y = -1;

  let finalSteering = p5.Vector.add(totalFlee.normalize(), wallPush.mult(2.5));
  
  if (finalSteering.mag() > 0) {
    finalSteering.normalize().mult(fleeSpeed);
    ryanPos.add(finalSteering);
  }

  // AVOID RUN OUT OF THE MAP
  ...
  if (s >= 23) {
    result = "YOU ARE OF FUEL,GAME OVER";
    stopAllMusic();
    state = "game over";
  }
}
